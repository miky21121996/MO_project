...
def Filter_Data(start_date, end_date, obs_file_path, path_to_accepted_metadata_obs_file):
    ...
    mask_time_period = (data[time_period].apply(lambda x: pd.to_datetime(datetime.datetime.strptime(x.split(',')[0], "%Y-%m-%d"))) <= start_date) & \
                    (data[time_period].apply(lambda x: pd.to_datetime(datetime.datetime.strptime(x.split(',')[1], "%Y-%m-%d"))) >= end_date)

    mask_field_list = (data[fields_list].str.contains('HCSP')) | \
                        (data[fields_list].str.contains('EWCT') & data[fields_list].str.contains('NSCT'))

    mask_num_sfc_levs = data[num_sfc_levs].astype(int) != 0

    filtered_data = data.loc[mask_time_period & mask_field_list & mask_num_sfc_levs]

    # get removed rows and reason for removal
    removed_data_mask = ~(mask_time_period & mask_field_list & mask_num_sfc_levs)
    ...
    print(filtered_data)
    filtered_data.to_csv(path_to_accepted_metadata_obs_file, sep=';', index=False)
...
def Mask_For_Depth(dataset):
    ...
    mask_range = (((dataset['DEPH'] >= 2) & (dataset['DEPH'] <= 4)) & (dataset['DEPH'] != 3))
    ...
    masked_difference_values.data[np.isnan(masked_difference_values.data).all(axis=1)] = np.inf
...
def Check_Quality_and_Filter(dataset, dataframe, quality_flag, mask_depth):
    ...
    if all([x in dataset.data_vars for x in check_vars]):
        ...
        for i in range(hcsp_filtered.shape[0]):
            ...
            value_hcsp = hcsp_filtered[i].values[np.isfinite(hcsp_filtered[i].values)]
            # If there are no non-NaN values, append NaN to the values list
            if len(value_hcsp) == 0:
                value_ewct = ewct_filtered[i].values[np.isfinite(ewct_filtered[i].values)]
                value_nsct = nsct_filtered[i].values[np.isfinite(nsct_filtered[i].values)]
            ...
            # Otherwise, append the non-NaN value to the values list
                ...
                value_hcdt = hcdt_filtered[i].values[np.isfinite(hcdt_filtered[i].values)]
                if len(value_hcdt) == 0:
                    dir_values.append(np.nan)
                else:
        ...
        for i in range(ewct_filtered.shape[0]):
            ...
            # Otherwise, append the non-NaN value to the values list
                ...
                values.append(np.sgqrt(value_ewct[0]^2 + value_nsct[0]^2))
                dir_values.append(value_ewct[0],value_nsct[0])
            # Append the time for this row to the times list
            times.append(ewct_filtered.TIME[i].values)
    return times, values, dir_values
...
def Remove_Outliers(dict_speed):
    ...
    for dict_key, dict_value in dict_speed.items():
        ...
        update_velocity[dict_key] = {'datetime': dict_value['datetime'], 'velocity': np.where((dict_value['velocity'] > (mean_value+2*sd_value)) | (
            dict_value['velocity'] < (mean_value-2*sd_value)), np.nan, dict_value['velocity'])}
    ...
    for i, (path_to_mod_output, path_to_destag_output_folder, path_to_mask) in enumerate(zip(path_to_mod_output_arr,path_to_destag_output_folder_arr, path_to_mask_arr)):
        ...
        for single_date in daterange(start_date, end_date, time_res[i][-1]):
            ...
            if time_res[i] == "1d":
                ...
                destaggered_U_current = destaggered_U_current.assign(destaggered_u=(('y', 'x'),destaggered_u))
                destaggered_U_current = destaggered_U_current.assign(nav_lon=(('y', 'x'),mesh_mask.glamt.values[0,:,:]))
                destaggered_U_current = destaggered_U_current.assign(nav_lat=(('y', 'x'),mesh_mask.gphit.values[0,:,:]))
                ...
                destaggered_V_current = destaggered_V_current.assign(destaggered_v=(('y', 'x'),destaggered_v))
                destaggered_V_current = destaggered_V_current.assign(nav_lon=(('y', 'x'),mesh_mask.glamt.values[0,:,:]))
                destaggered_V_current = destaggered_V_current.assign(nav_lat=(('y', 'x'),mesh_mask.gphit.values[0,:,:]))
...
def Find_Nearest_Mod_Point(obs_file, ini_date, name_exp, time_res, path_to_destag_output_folder, depth_obs, last_lat, last_lon, t_mask, mod_grid):
    ...
    for name, obs in obs_file.items():
        ...
        if mod_grid == "regular":
            ...
            i = np.where(U_current.nav_lon.values==nemo_lonlat[idx_near_obs][0][1])[1]
            j = np.where(U_current.nav_lat.values==nemo_lonlat[idx_near_obs][0][0])[0]
        else:
...
def Save_Mod_Ts(obs_file, start_date, end_date, name_exp, date_in, date_fin, temp_res, path_to_destag_output_folder, nearest_point, path_to_output_mod_ts_folder, mod_grid, time_res_to_average, name_stat, CMEMS_code, WMO_code, path_to_obs_file):
    ...
    for name, obs in obs_file.items():
        ...
        if temp_res == "1d":
            averaged_time=dataset_obs['TIME'].resample(TIME="1D").mean(skipna=True)
        if temp_res == "1h":
            averaged_time=dataset_obs['TIME'].resample(TIME="1H").mean(skipna=True)
        averaged_time=averaged_time.sel(TIME=slice(start_date, end_date))
        
        # set the datetime column as the index of the DataFrame
        print("mod file name: ", mod_file[name])
        mod_ds = pd.DataFrame({'datetime': averaged_time['TIME'].data[:], 'velocity': mod_file[name], 'direction': mod_dir_file[name]})
        ...
        if not np.isnan(averaged_mod_velocities).all():
            print("sono qui")
            mod_dict[array_name[boolArr][0]] = {'datetime': mod_time_file[name], 'velocity': mod_file[name], 'direction': mod_dir_file[name]}
            averaged_mod_dict[array_name[boolArr][0]] = {'datetime': averaged_mod_times, 'velocity': averaged_mod_velocities, 'direction': averaged_mod_directions}
        
        averaged_mod_ds = xr.Dataset(data_vars=dict(TIME=(['TIME'], averaged_mod_dict[array_name[boolArr][0]]['datetime'][:]),mod_vel=(['TIME'], averaged_mod_dict[array_name[boolArr][0]]['velocity'][:]),mod_direction=(['TIME'],averaged_mod_dict[array_name[boolArr][0]]['direction'][:])))
        #averaged_mod_ds=mod_ds.resample(TIME=time_res_to_average).mean(skipna=True)
        averaged_mod_ds.to_netcdf(path=path_to_output_mod_ts_folder + output_mod_file)
