...
def main(args):
    ...
    for i, mod_folder in enumerate(path_to_mod_ts_arr):
        if time_res_arr[i][1]=='M':
            timerange = pd.date_range(start_date, end_date, freq=time_res_arr[i][1]) - pd.DateOffset(days=15)
        elif time_res_arr[i][1]=='D':
            timerange = pd.date_range(start_date, end_date, freq=time_res_arr[i][1]) - pd.DateOffset(hours=12)
        
        onlyfiles_mod = [f for f in sorted(listdir(mod_folder)) if isfile(join(mod_folder, f))]
        onlyfiles_obs = [f for f in sorted(listdir(path_to_obs_ts)) if isfile(join(path_to_obs_ts, f))]
    ...
    for key_obs_file, name_stat in zip(sorted(obs_file.keys()),vel_mod_ts[0].keys()):
        ...
        plot_mod_obs_ts_diff_comparison(name_stat, date_in, date_fin, time_res_arr, obs_file, key_obs_file, vel_mod_ts, vel_obs_ts, label_plot_arr, timerange, time_res_axis, path_to_comparison)
        plot_mod_obs_ts_comparison(name_stat, date_in, date_fin, time_res_arr, obs_file, key_obs_file, vel_mod_ts, vel_obs_ts, label_plot_arr, timerange, time_res_axis, path_to_comparison)
        ii = np.isfinite(np.array(vel_obs_ts[name_stat]))
        plot_mod_obs_ECDF_comparison(ii,name_stat, date_in, date_fin, time_res_arr, obs_file, key_obs_file, vel_mod_ts, vel_obs_ts, label_plot_arr, path_to_comparison,'_mod_obs_ECDF_comparison.png')
        ...
        for i in range(len(label_plot_arr)):
            min_vel_dict = np.nanmin(vel_mod_ts[i][name_stat][~np.isnan(vel_obs_ts[name_stat])][~np.isnan(a)])
            min_mod_mooring_vel = min(min_mod_mooring_vel, min_vel_dict)
            max_vel_dict = np.nanmax(vel_mod_ts[i][name_stat][~np.isnan(vel_obs_ts[name_stat])][~np.isnan(a)])
            max_mod_mooring_vel = max(max_mod_mooring_vel, max_vel_dict)
            
        min_mooring_vel[name_stat] = min(np.nanmin(b[~np.isnan(a)]),min_mod_mooring_vel)
        max_mooring_vel[name_stat] = max(np.nanmax(b[~np.isnan(a)]),max_mod_mooring_vel)
        #if not list(a):
        ...
        ax.bar(a[~np.isnan(a)], b[~np.isnan(a)], normed=True, bins=np.linspace(min_mooring_vel[name_stat],max_mooring_vel[name_stat],5),opening=0.8,edgecolor='white',cmap=turbo)
    ...
    plot_bias_ts_comparison(date_in, date_fin, time_res_arr, timerange, label_plot_arr, bias_ts, time_res_axis, path_to_comparison)
    plot_rmse_ts_comparison(date_in, date_fin, time_res_arr, timerange, label_plot_arr, rmsd_ts, time_res_axis, path_to_comparison)
    ...
    for key_obs_file, name_stat in zip(sorted(obs_file.keys()),vel_mod_ts[0].keys()):
        for exp in range(len(label_plot_arr)):
            ...
            if exp==0:
                sdev[name_stat]=list(np.around(np.array([taylor_stats['sdev'][0],taylor_stats['sdev'][1]]),4))
                crmsd[name_stat]=list(np.around(np.array([taylor_stats['crmsd'][0],taylor_stats['crmsd'][1]]),4))
                ccoef[name_stat]=list(np.around(np.array([taylor_stats['ccoef'][0],taylor_stats['ccoef'][1]]),4))
            else:
                append_value(sdev, name_stat, round(taylor_stats['sdev'][1],4))
                append_value(crmsd, name_stat, round(taylor_stats['crmsd'][1],4))
                append_value(ccoef, name_stat, round(taylor_stats['ccoef'][1],4))
    ...
    string_time_res = Get_String_Time_Resolution(start_date, end_date, time_res_arr[0])
    for exp in range(len(label_plot_arr)):
        statistics[exp] = {}
        len_not_nan_values=[]
        moor_names=[]
        for counter, (key_obs_file, name_stat) in enumerate(zip(sorted(obs_file.keys()),vel_mod_ts[exp].keys())):
            ...
            plotname = label_plot_arr[exp] + '_' + name_stat + '_' + date_in + '_' + date_fin + '_' + time_res_arr[exp] +  '_qqPlot.png'
            ...
            statistics_array=scatterPlot(np.array(vel_mod_ts[exp][name_stat]),np.array(vel_obs_ts[name_stat]),path_to_output_experiments_arr[exp] + plotname,label_plot_arr[exp],1,len(vel_mod_ts[exp][name_stat]),possible_markers[counter],name_stat,possible_colors,string_time_res,title=title,xlabel=xlabel,ylabel=ylabel)
            ...
            name_file_substring="_" + label_plot_arr[exp] + "_" + name_stat + "_windrose"
            ...
            mask = np.isreal(a) & np.isreal(b) & np.isreal(c) & np.isreal(d) & ~np.isnan(a) & ~np.isnan(b) & ~np.isnan(c) & ~np.isnan(d)
            print("lunghezza mod: ", b[mask].shape)
            ax = WindroseAxes.from_ax()
            turbo = plt.get_cmap('turbo')
            ax.bar(a[mask], b[mask], normed=True, bins=np.linspace(min_mooring_vel[name_stat],max_mooring_vel[name_stat],5),opening=0.8,edgecolor='white',cmap=turbo)
        ...
        plotname = label_plot_arr[exp] + '_' + date_in + '_' + date_fin + '_' + time_res_arr[exp] +  '_qqPlot.png'
        title = 'Surface (3m) Current Velocity -ALL \n Period: ' + date_in + '-' + date_fin
        xlabel = 'Observation Current Velocity [m/s]'
        ylabel = 'Model Current Velocity [m/s]'
        statistics_array=scatterPlot(mod_array,obs_array,path_to_output_experiments_arr[exp] + plotname,label_plot_arr[exp],len(onlyfiles_mod),timerange.shape[0],possible_markers,moor_names,possible_colors,string_time_res,len_not_nan_values=len_not_nan_values,title=title,xlabel=xlabel,ylabel=ylabel)
        row_all = mean_all + statistics_array
        statistics[exp]["ALL BUOYS"] = row_all

        a_file = open(path_to_output_experiments_arr[exp] + "statistics_" + label_plot_arr[exp] + "_" + date_in + "_" + date_fin + ".csv", "w")
    ...
    for key_obs_file, name_stat in zip(sorted(obs_file.keys()),vel_mod_ts[0].keys()):
        ...
        a=a[~np.isnan(b)]
        b=b[~np.isnan(b)]
        min_value = max(ymin[name_stat].values())
        ...
        plot_windrose(a[~np.isnan(a)],b[~np.isnan(a)],min_mooring_vel[name_stat], max_mooring_vel[name_stat],date_in,date_fin,name_file_substring,title_substring,path_to_comparison,ymin[name_stat][min_key],ymax[name_stat][max_key])
        ...
        for exp in range(len(label_plot_arr)):
            ...
            plot_windrose(a[mask],b[mask],min_mooring_vel[name_stat], max_mooring_vel[name_stat],date_in,date_fin,name_file_substring,title_substring,path_to_output_experiments_arr[exp],ymin[name_stat][min_key],ymax[name_stat][max_key])
