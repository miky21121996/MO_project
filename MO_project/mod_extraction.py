...
def main(args):
    ...
    with open(path_to_metadata_obs_file) as f:
        ...
        reader = csv.reader(filter(lambda row: row and row[-1] != '', (line.rstrip(
        ).strip(';') for line in f if not line.startswith('#'))), delimiter=';')
    ...
    for i, (mask_file, input_mod_folder) in enumerate(zip(path_to_mask_arr, path_to_input_mod_folder_arr)):
        ...
        nearest_point = {}
        nearest_point = Find_Nearest_Mod_Point(
            obs_file, start_date, name_exp_arr[i], time_res_model_arr[i], input_mod_folder, depth_obs, last_lat, last_lon, t_mask, mod_grid_arr[i])
        mod_dict, averaged_mod_dict = Save_Mod_Ts(obs_file, start_date, end_date, name_exp_arr[i], date_in, date_fin, time_res_model_arr[i], input_mod_folder,
                                                  nearest_point, path_to_out_mod_ts_arr[i], mod_grid_arr[i], time_res_model_to_average_arr[i], name_stat, CMEMS_code, WMO_code, path_to_obs_file)

        for dict_key, dict_value in averaged_mod_dict.items():
            fig, (ax1, ax2) = plt.subplots(2)
            fig.subplots_adjust(hspace=1)
            fig.suptitle('Mooring: {} (model experiment {})'.format(
                dict_key, name_exp_arr[i]))
            ...
            fig_dir.suptitle('Mooring: {} (model experiment {})'.format(
                dict_key, name_exp_arr[i]))
