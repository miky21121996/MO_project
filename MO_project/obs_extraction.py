...
def main(args):
    ...
    for index, row in df.iterrows():
        ...
        times, speed_values, dir_values = Check_Quality_and_Filter(
        ...
        if selected_name.empty:
            selected_name_1 = station_info.str.extract(r'(\d+)').astype(float).values
            selected_name_1 = [item for sublist in selected_name_1 for item in sublist]
        ...
        dict_speed[selected_name_end] = {'datetime': times, 'velocity': speed_values, 'direction': dir_values}
    ...
    for count, (dict_key, dict_value) in enumerate(removed_outliers_dict.items()):
        ...
        df_rem_out_speed = pd.DataFrame({'datetime': dict_value['datetime'][:], 'velocity': dict_value['velocity'][:], 'direction': dict_speed[dict_key]['direction'][:]})
        ...
        df_rem_out_speed_resampled = df_rem_out_speed.resample(time_res_to_average).mean()
        ...
        if not np.isnan(daily_velocities).all():
            ...
            if bool_nan:
                daily_dict[dict_key] = {'datetime': daily_times, 'velocity': daily_velocities, 'direction': daily_directions}
                end_filtered_df.loc[count] = df.loc[count]
    
    end_filtered_df.to_csv(path_to_accepted_metadata_obs_file, sep=';', index=False)

    for dict_key, daily_dict_value in daily_dict.items():
        fig, (ax1, ax2, ax3) = plt.subplots(3)
        fig.subplots_adjust(hspace=1)
        fig.suptitle('Mooring: {}'.format(dict_key))
        ax1.plot(dict_speed[dict_key]['datetime'][:], dict_speed[dict_key]['velocity'][:])
        ax1.tick_params(axis='x', rotation=45, labelsize=7) 
        ax2.plot(removed_outliers_dict[dict_key]['datetime'][:], removed_outliers_dict[dict_key]['velocity'][:])
        ax2.tick_params(axis='x', rotation=45, labelsize=7) 
        ax3.plot(daily_dict_value['datetime'][:],daily_dict_value['velocity'][:])
        ...
        ax1_dir.plot(dict_speed[dict_key]['datetime'][:], dict_speed[dict_key]['direction'][:])
        ax1_dir.tick_params(axis='x', rotation=45, labelsize=7) 
        ax2_dir.plot(daily_dict_value['datetime'][:],daily_dict_value['direction'][:])
