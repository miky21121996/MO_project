...
def main(args):
    ...
    for index, _ in df.iterrows():
        ...
        mask_depth = Mask_For_Depth(ds_restricted, depth_obs)
        ...
        print("name station: ", selected_name_end)
    ...
    for count, (dict_key, dict_value) in enumerate(removed_outliers_dict.items()):
        ...
        if not np.isnan(daily_velocities).all():
            bool_nan = Check_Nan(daily_velocities, nan_treshold)
            if bool_nan:
                if np.size(dict_value['EWCT'][:]) > 0 and np.size(dict_value['NSCT'][:]) > 0:
                    daily_dict[dict_key] = {
